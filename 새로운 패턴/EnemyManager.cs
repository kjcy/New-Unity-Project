...
public class EnemyManager : MonoBehaviour
{
    ...
    IEnumerator PattenStage1Cor()
    {
        do
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                temp[i] = Instantiate(barragebox[0], mainBoss.transform.position + new Vector3(1, -1.3f, 0), Quaternion.identity, barrageParent);
            }
            temp[4] = Instantiate(barragebox[1], mainBoss.transform.position + new Vector3(1, -1.3f, 0), Quaternion.identity, barrageParent);

            for (int i = 0; i < 5; i++)
            {
                temp[i].GetComponent<Enemy>().MoveEnemy(mainBoss.transform.position + new Vector3(-20, -1.3f, 10), 100f);
                yield return new WaitForSecondsRealtime(0.95f);
            }
            yield return new WaitForSecondsRealtime(3f);//3초 뒤에 다음 패턴
            ...
            for (int i = 0; i < 3; i++)
            {
                temp[0] = Instantiate(barragebox[i % 2], new Vector3(7, 10, 0), Quaternion.identity, barrageParent);
                temp[1] = Instantiate(barragebox[(i + 1) % 2], new Vector3(7, 10, 0), Quaternion.identity, barrageParent);
